...
function loadPdfJs() {
  ...
  pdfJsPromise = new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error("PDF 처리 모듈을 불러오지 못했습니다. 인터넷 연결을 확인해주세요."));
    document.head.append(script);
  });
  return pdfJsPromise;
}

async function loadPdfCanvases(file) {
...
async function loadScheduleCanvases(file) {
  if (isPdfFile(file)) return loadPdfCanvases(file);
...
async function buildScheduleFromPhotos() {
  ...
  try {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      const canvases = await loadScheduleCanvases(file);
      ...
      canvases.forEach((rawCanvas, pageIndex) => {
        ...
        const section = inferred.section || (files.length === 1 && canvases.length === 1
          ? "시간표"
          : SCHEDULE_SECTION_LABELS[images.length] || `시간표 ${images.length + 1}`);
        images.push({
